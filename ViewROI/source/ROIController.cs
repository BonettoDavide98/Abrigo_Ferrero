...
namespace ViewROI
{
    ...
    public class ROIController
    {
        ...
        public const int EVENT_DEACTIVATED_ROI = 57;
        ...
        /// 'Seed' ROI object forwarded by the application forms class.
        /// </param>
        public void setROIShape(ROI r, int id)
        {
            roiMode = r;
            roiMode.setRoiId(id);
            roiMode.setOperatorFlag(stateROI);
        ...
        public int mouseDownAction(double imgX, double imgY)
        {
            ...
            {
                bool notificaDeselezione = false;
                if (activeROIidx >= 0)
                {
                    notificaDeselezione = true;
                }
                ...
                if (idxROI >= 0)
                {
                    ...
                    notificaDeselezione = false;
                }

                if (notificaDeselezione)
                {
                    NotifyRCObserver(ROIController.EVENT_DEACTIVATED_ROI);
                }
                // in test			viewController.repaint();
            }
            return activeROIidx;
        }
