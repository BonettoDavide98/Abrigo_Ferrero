...
namespace ViewROI
{
    ...
    public class HWndCtrl
    {
        ...
        public bool ShowOnlyImages = false;
        public bool EnableMiddleMove = true;
        public bool ShowStringMessage = true;
        ...
        /// </summary>
        private const int MAXNUMOBJLIST = 500;
        ...
        private void setImagePart(ICogImage image)
        {
            string s;
            int w, h;
        ...
        private void mouseMoved(object sender, HalconDotNet.HMouseEventArgs e)
        {
            ...
            if (roiManager != null && (roiManager.activeROIidx != -1) && (dispROI == MODE_INCLUDE_ROI) && e.Button == MouseButtons.Left)
            {
                roiManager.mouseMoveAction(e.X, e.Y);
            }
            else if (stateView == MODE_VIEW_MOVE || (e.Button == MouseButtons.Middle && EnableMiddleMove))
            {
                motionX = ((e.X - startX));
                motionY = ((e.Y - startY));
        ...
        public void zoomByGUIHandleScaled(double scale)
        {
            double x, y;
        ...
        public void zoomByGUIHandleScaled(double scale, double xMouse, double yMouse)
        {
            double x, y;
        ...
        public void repaint(HalconDotNet.HWindow window)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ((ShowOnlyImages && entry.HObj is ICogImage) || !ShowOnlyImages)
                {
            ...
            if (!ShowOnlyImages)
                for (int i = 0; i < HTextList.Count; i++)
                {
                    HTextEntry entry2 = (HTextEntry)HTextList[i];
                    window.SetFont(entry2.Font);
                    //window.SetTposition(entry2.Row, entry2.Column);
                    //window.SetColor(entry2.Color);
                    //window.WriteString(entry2.Text);

                    window.DispText(entry2.Text, "window", entry2.Row, entry2.Column, entry2.Color, new HTuple("box", "box_color"), new HTuple("false", "black"));
                    //window.DispText(entry2.Text, "image", entry2.Row, entry2.Column, entry2.Color, new HTuple("box", "box_color"), new HTuple("true", "black"));
                }
        ...
        public void addIconicVar(HObject obj)
        {
            ...
            if (obj is ICogImage)
            {
                ...
                area = ((ICogImage)obj).GetDomain().AreaCenter(out r, out c);
                ((ICogImage)obj).GetImagePointer1(out s, out w, out h);
        ...
        public void addTextVar(HTextEntry obj)
        {
            HTextList.Add(obj);
        }
        ...
        public void clearList()
        {
            ...
            {
                (HObjList[i] as HObjectEntry).clear();
            }
            HObjList.Clear();
